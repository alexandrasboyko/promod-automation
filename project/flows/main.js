...
async function checkThatAfterFailedLoginFieldsAreFilled(userData) { // функция проверки, что после неудачного логина в систему, поля остаются заполненными
  // возвращаемый, из getData результат, присваиваем деструктуризированному объекту {login}
  const {login} = await main.getData({login: fieldsToNull(userData)}) // new mainPage {login:userdata{
  // username:"kmfm"
  // password:"vkmkdnk"}

  Object.keys(userData).forEach((key) => {
    expect(userData[key]).toEqual(login[key])
  })

  // проверка значения каждого ключа объекта, что был передан в функцию, на соответстие переданного в функцию значения ключа объекта тому значению ключа объекта, что был извлечен с помощью метода "getData" и записан в {login}
